...
function kafkaOptions(environment: ReturnType<typeof getEnvironment>) {
  const sasl =
...
@Injectable()
export class EvidenceOutboxPublisher implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(EvidenceOutboxPublisher.name);
  ...
  async flush() {
    ...
    for (const row of rows) {
      try {
          ...
          messages: [
            {
              ...
              headers: {
                ...
                ...(row.causationId
                  ? { "x-causation-id": row.causationId }
                  : {}),
              },
            },
          ],
        ...
        this.logger.error({
          event: "evidence.outbox.publish.failed",
          eventId: row.id,
